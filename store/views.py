...
def create_checkout_session(request, order_id):
    ...
    # Reload .env so newly saved keys take effect immediately without needing server restart
    load_dotenv(settings.BASE_DIR / '.env', override=True)
    secret_key = os.environ.get('STRIPE_SECRET_KEY', getattr(settings, 'STRIPE_SECRET_KEY', ''))
    
    if not secret_key or 'your_stripe_secret_key_here' in secret_key:
        messages.error(
            request,
            "Stripe Key Warning: The placeholder key 'sk_test_your_stripe_secret_key_here' is still in your .env file. "
            "Please paste your real Stripe secret key into .env and save the file (Ctrl + S)."
        )
        return redirect('store:dummy_payment', order_id=order.id)
...
def mark_as_paid(request, order_id):
    ...
    order.paid = True
    order.save()
    messages.success(request, f'Demo payment of ₹{order.total} completed successfully!')
    return redirect('store:order_success', order_id=order.id)
